...
#define UPTODOWN 1
#define DOWNTOUP 0
#define VEC_CHECK(vec, num) if(!CHECK(vec, num)) {return nil;}
static Mat mask;
static Mat mask2;
...
@protected
    UIImage * frontEdge;
    UIImage * frontEdgeFilled;
    UIImage * flankEdge;
    UIImage * flankEdgeFilled;
...
template <class T>
bool CHECK(vector<T> vec, int num) {
    return vec.size() >= num;
}
...
//        img2 = [self resizeImg:img2 withWidth:800];
        ...
        if (scene1.size() == 0 || scene2.size() == 0) {
            [_delegate onProcessed:nil];
            return nil;
        }
        Mat H = findHomography(scene2, scene1, RANSAC);
        if (H.data == nullptr || H.cols != 3 || H.rows != 3) {
            [_delegate onProcessed:nil];
            return nil;
        }
...
//    equalizeHist(result, result);
    ...
    Mat edge_contours_filled(result.size(),CV_8U,Scalar(0));
...
//    
    ...
    drawContours(edge_contours, true_contours, -1, Scalar(255), 1);
    drawContours(edge_contours_filled, true_contours, -1, Scalar(255), FILLED);
    [_delegate onUpdate:MatToUIImage(edge_contours) withProgress:8];
    if (type == 1) {
        frontEdgeFilled = MatToUIImage(edge_contours_filled);
        frontEdge = MatToUIImage(edge_contours);
    } else {
        flankEdgeFilled = MatToUIImage(edge_contours_filled);
...
int findMin(Mat contour_filled, const int& start_y, const int& end_y)
{
    if (contour_filled.rows < contour_filled.cols) {
        ...
        flip(contour_filled, contour_filled, 2);
    }
    int min = INT_MAX;
    int min_pos_y = 0;
...
- (UIImage *) flankAnalyze
{
    @try {
        ...
        Mat result, contour, contour_filled;
        
        UIImageToMat(flankImage, result, false);
        UIImageToMat(flankEdge, contour, false);
        UIImageToMat(flankEdgeFilled, contour_filled, false);
//        Mat contour_filled(contour.size(),CV_8U,Scalar(0));
        ...
        flankRatio = getRatio(MatToUIImage(contour), _height);
...
//        drawContours(contour_filled, contours, -1, Scalar(255), FILLED);
    ...
    //    max_pos_x = findMaxX(contour_filled, max_pos_x, min_pos_x);
        ...
        VEC_CHECK(sleeve_mid_pts, 2);
    //    line(result, sleeve_mid_pts[0], sleeve_mid_pts[1], Scalar(255), 3);
        sleeve_mid_flank = abs(sleeve_mid_pts[0].y - sleeve_mid_pts[1].y) * flankRatio;
        ...
        VEC_CHECK(sleeve_pts, 2);
    //    line(result, sleeve_pts[0], sleeve_pts[1], Scalar(255), 3);
        sleeve_flank = abs(sleeve_pts[0].y - sleeve_pts[1].y) * flankRatio;
...
//        }
        ...
        int neck_to_top_pixel = neck_to_top / flankRatio + r.tl().y;
        neck_pts = crossPointsY(findMin(contour_filled, neck_to_top_pixel - 5 / flankRatio, neck_to_top_pixel + 5 / flankRatio), contour);
        VEC_CHECK(neck_pts, 2);
    //    line(result, neck_pts[1], neck_pts[0], Scalar(255), 3);
        neck_flank = (neck_pts[neck_pts.size()-1].x - neck_pts[0].x) * flankRatio;
...
//        line(result, shoulder_pts_left[0], shoulder_pts_left[1], Scalar(255, 0, 0), 3);
        ...
        min_pos_y = findMin(contour_filled, shoulder_pts_left[1].y + r.height / 6 - 5 / flankRatio,
                            shoulder_pts_left[1].y + r.height / 6 + 5 / flankRatio);
        waist_pts = crossPointsY(min_pos_y, contour);
        VEC_CHECK(waist_pts, 2);
...
//        crotch_flank = (crotch_pts[crotch_pts.size()-1].x - crotch_pts[0].x) * flankRatio;
        ...
        // hip
        Mat waist_to_down = contour(Range(waist_pts[0].y, waist_pts[0].y + 30/flankRatio), Range(0, frontImage.size.width - 1));
...
//        }
        VEC_CHECK(hip_right_pt, 1);
        hip_pts = crossPointsY(hip_right_pt[0].y + waist_pts[0].y, contour);
        VEC_CHECK(hip_pts, 2);
        line(result, hip_pts[0], hip_pts[hip_pts.size()-1], Scalar(0, 0, 255), 3);
        hip_flank = (hip_pts[hip_pts.size()-1].x - hip_pts[0].x) * flankRatio;
        hip_to_top = (hip_pts[0].y - r.tl().y) / (r.height * 1.0f);
...
- (UIImage *) frontAnalyze
{
    @try {
        ...
        Mat contour, result, contour_filled;
        int up, middle, down;
        
        
        UIImageToMat(frontImage, result, false);
        UIImageToMat(frontEdge, contour, false);
        UIImageToMat(frontEdgeFilled, contour_filled, false);
        if (frontImage.imageOrientation == UIImageOrientationRight) {
            transpose(result, result);
            flip(result, result, 1);
            transpose(contour, contour);
        ...
        frontRatio = getRatio(MatToUIImage(contour), _height);
        cv::Rect r = boundingRect(contour);
...
//        int end_y = start_y-20/ratio;
    ...
    //    std::cout << "肩宽: " << abs(candidates[0].x-candidates[1].x) * ratio << std::endl;
        ...
        VEC_CHECK(shoulder_pts_left, 2);
        VEC_CHECK(shoulder_pts_right, 2);
        line(result, shoulder_pts_left[0], shoulder_pts_right[0], Scalar(0,255,0), 3);
        shoulder_low_to_top = getDistance(shoulder_pts_left[0], shoulder_pts_right[0]) * frontRatio;
        ...
        bust_len_front = (bust_pts[1].x - bust_pts[0].x) * frontRatio;
...
//        min_pos_x = pushToMin(contour_filled, arm_left, 5, LEFTTORIGHT);
        ...
        sleeve_mid_front = abs(sleeve_mid_pts[0].y - sleeve_mid_pts[1].y) * frontRatio;
        ...
        sleeve_front = abs(sleeve_pts[0].y-sleeve_pts[1].y) * frontRatio;
